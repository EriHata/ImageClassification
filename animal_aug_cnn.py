import keras
from keras.models import Sequential                         # NNのモデルを作る際に使用
from keras.layers import Conv2D, MaxPooling2D            # NNの部品
from keras.layers import Dropout, Flatten, Dense    # NNの部品　Denseは全結合層
...
def main():
	...
	# 正規化
	#X_train = X_train / 256  # 正規化
	#X_test = X_test / 256    # 正規化
...
def model_train(X, y):
	...
	model.add(Dense(3, activation='softmax'))              # 最終出力のクラス数
	model.add(Activation('softmax'))    # 確率にするためにsoftmax 　
